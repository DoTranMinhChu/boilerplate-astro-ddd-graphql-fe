...
import type { NavLink } from '@/shared/services/siteSettings/siteSettings.service';
 ...
 * CmsPageShell.astro. Content comes from `getSiteSettings` (admin-editable —
 * see the "Header & Footer" settings page); falls back to sensible defaults when
 * nothing has been configured yet, so the site never renders with an empty nav.
...
export function SiteHeader(props: { currentPath: string; logoText?: string; navLinks?: NavLink[] }) {
    ...
    return (
        ...
        >
            <div class="mx-auto flex h-16 max-w-[1720px] items-center justify-between px-[4.5vw] text-[#f2f2f2]">
                <a href="/trang-chu" class="text-2xl font-medium tracking-tight">{logoText()}</a>
