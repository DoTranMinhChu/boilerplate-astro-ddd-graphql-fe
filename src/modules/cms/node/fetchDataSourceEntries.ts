...
// CONTENT_GRID Section cũ dùng (props.node.props.dataSource, KHÔNG đổi shape — spec §3), qua
// createResource — KHÔNG cần Section's cơ chế SSR-resolve-thành-props riêng (`props.section.
// entries`), vì Phase 0 M2c đã kiểm chứng THẬT (curl trang thật, `CMS_NODE_TREE_ENABLED=true`):
// `@astrojs/solid-js` (bản đang dùng, không override) tự bọc mọi component SSR trong <Suspense> +
// `renderToStringAsync`, nên `createResource` này ĐÃ render đủ dữ liệu ở SSR HTML — không trắng,
// không cần refactor thêm. Tách 1 hàm dùng CHUNG 3 nơi — tránh lặp cùng logic mode manual/dynamic
// + resolveGenericDataSource 3 lần (cùng nguyên tắc DRY nodeDataBinding.ts's fetchRepeatEntries
// 'own' branch đã áp dụng).
