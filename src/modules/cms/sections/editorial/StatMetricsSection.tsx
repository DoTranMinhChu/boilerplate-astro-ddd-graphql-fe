...
export interface StatMetricsContent {
    heading?: string;
    metrics?: StatMetric[];
}
...
export function StatMetricsSection(props: { section: ResolvedSection }) {
    const content = () => (props.section.content || {}) as StatMetricsContent;
    const metrics = () => content().metrics || [];

    return (
        <section class="bg-[#020202] py-20 text-[#f2f2f2]">
            <div class="mx-auto max-w-[1720px] px-[5vw]">
                <h2 use:animate={getLayer(props.section, 'heading')} class="m-0 mb-16 text-center font-light" style={{ 'font-size': 'clamp(32px, 3.5vw, 56px)' }}>
                    {content().heading}
                </h2>
                <div use:animate={getLayer(props.section, 'metrics')} class="grid grid-cols-2 gap-10 md:grid-cols-4">
                    <For each={metrics()}>
                        {(m) => (
