...
} from '@modules/cms/node/commands/nodeCommands';
...
describe('createUpdateNodePropertyCommand', () => {
    ...
    it('SAVABLE_NODE_FIELD_KEYS matches the BE\'s real UpdateNodeInput field list (independent-source-of-truth completeness guard, not a self-referential one)', async () => {
        ...
        // test/ mirrors src/ path-for-path, so swap the leading segment to find the real source.
        const here = dirname(fileURLToPath(import.meta.url)).replace(`${sep}test${sep}`, `${sep}src${sep}`);
