...
describe('assignDefaultGridPositions', () => {
    it('keeps already-placed fields exactly as they are', () => {
        const existing = [{ fieldKey: 'title', colStart: 1, colSpan: 6, rowStart: 0, rowSpan: 1 }];
        const result = assignDefaultGridPositions(FIELDS, existing);
        expect(result.find((r) => r.fieldKey === 'title')).toEqual(existing[0]);
    });

    it('appends unplaced fields as full-width, rowSpan-1 rows after the max already-occupied row', () => {
        const existing = [{ fieldKey: 'title', colStart: 1, colSpan: 6, rowStart: 2, rowSpan: 3 }];
        const result = assignDefaultGridPositions(FIELDS, existing);
        const price = result.find((r) => r.fieldKey === 'price')!;
        const description = result.find((r) => r.fieldKey === 'description')!;
        // title occupies rows 2,3,4 (rowStart 2 + rowSpan 3) -> next free row is 5.
        expect(price).toEqual({ fieldKey: 'price', colStart: 1, colSpan: 12, rowStart: 5, rowSpan: 1 });
        expect(description).toEqual({ fieldKey: 'description', colStart: 1, colSpan: 12, rowStart: 6, rowSpan: 1 });
    });

    it('starts at row 0 when nothing is placed yet', () => {
        const result = assignDefaultGridPositions([{ key: 'title' }] as any, []);
        expect(result).toEqual([{ fieldKey: 'title', colStart: 1, colSpan: 12, rowStart: 0, rowSpan: 1 }]);
    });

    it('drops placements for fields that no longer exist on the content type', () => {
        const existing = [{ fieldKey: 'deletedField', colStart: 1, colSpan: 6, rowStart: 0, rowSpan: 1 }];
